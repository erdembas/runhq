  ...
  return (
    <details className="border-border border-b px-4 py-2 text-[11px]">
      <summary className="text-fg-muted flex cursor-pointer list-none items-center gap-2">
        ...
        >
          {error ? 'Check settings' : enabled ? 'On' : 'Off'}
        </span>
      </summary>
      <div className="mt-3 space-y-3 pb-1">
        <p className="text-fg-dim">
          Get updates when tasks need a decision, fail or finish. Notifications include no task,
          project or message content.
        </p>
        <button
          type="button"
        ...
        {!available && <p className="text-fg-dim">Available in the desktop app.</p>}
        {projectName ? (
          <label className="text-fg-muted flex items-center gap-2">
            ...
            />
            Mute notifications for {projectName}
          </label>
        ) : (
          <p className="text-fg-dim">Choose a project above to mute its notifications.</p>
        )}
        {error && (
