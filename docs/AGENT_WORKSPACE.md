...
7. **Changes** mevcut çalışma dizininin tracked Git diff'ini gösterir. Bu görünüm önceden var olan değişiklikleri de içerir; agent'a ait değişiklikler olarak etiketlenmez. Codex'in turn diff olayları ayrıca konuşmada tutulur.
...
Kuyruk sekme veya proje değiştirildiğinde çalışmaya devam eder. Hata, bağlantı kesintisi veya **Stop** sonrasında otomatik ilerleme durur; devam etmek için kuyruktaki devam/yeniden deneme eylemi kullanılır. Gönderim hatasında aynı istek kimliği korunur. Kuyruk uygulama belleğindedir: RunHQ tamamen kapanınca veya arayüz yeniden yüklenince gönderilmemiş mesajlar temizlenir; kaydedilmiş konuşma geçmişi korunur.
...
Destek kapsamı sağlayıcıya ve kurulu CLI/SDK sürümüne bağlıdır. Harici pencerelerde önceden açılmış konuşmaları içe aktarma, sağlayıcılar arasında konuşma aktarımı, dosya/görsel ekleme arayüzü, terminalin tüm slash komutlarına özel ekranlar, uzaktaki makineler ve uygulama kapandıktan sonra çalışan daemon kapsam dışıdır. Yeni protokol türleri için adapter genişletilmelidir; bilinmeyen olaylardan sahte başarı üretilmez.
...
- Plan/Canvas düzenlemeleri uygulamanın yerel web depolamasında tutulur; depolama hatası arayüzde bildirilir. Gönderilmemiş mesaj kuyruğu kalıcı veritabanına yazılmaz.
- Uygulama tamamen kapandığında aktif işler devam ettirilmez. Bir sonraki açılışta **Interrupted** görünür; pending onaylar temizlenir ve mesaj otomatik yeniden yürütülmez. Pencereyi gizlemek mevcut RunHQ pencere davranışına tabidir.
...
Yeni worktree, seçili projenin repo kökündeki committed `HEAD` üzerinden `codex/runhq-<id>` dalında açılır. Monorepo alt dizini korunur. Uncommitted değişiklikler, `.env` dosyaları veya `node_modules` kopyalanmaz. Ortam kurulumunu görev içinde veya terminalde yapın. Worktree'ler `RUNHQ_HOME/worktrees` altında korunur; otomatik merge, commit, push veya silme yapılmaz.
...
- `apps/desktop/src-tauri/src/ipc/agents.rs`: ince Tauri komut katmanı.
- `apps/desktop/src/store/useAgentStore.ts`: canlı oturum listesi ve görünümden bağımsız taslaklar.
- `apps/desktop/src/components/agents`: proje listesi, sohbet, yeni görev, diff ve terminal bağlantıları.
