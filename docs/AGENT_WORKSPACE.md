...
**Workflows**, bir görevi senin seçtiğin adımlar → kontroller → açık onay zinciri olarak yürütür. Her iş akışı hedefini (**objective**) ve kabul ölçütünü (**acceptance**) taşır, izole bir checkout'ta çalışır ve başladığı commit'i kaydeder. Adımlar kendiliğinden ilerlemez; otomatik ilerleme isteğe bağlıdır ve sınırlıdır.

Adım listesi iş akışı oluşturulurken belirlenir: her adım rolünü (**plan**, **implement**, **review**, **revise**, **validate**), onu çalıştıran hesabı ya da havuzu, modelini ve girdi olarak aldığı adımı taşır. İlk adım iş üretmek zorundadır — inceleyecek bir şey olmadan inceleme anlamsızdır — ve entegrasyondan önce en az bir bağımsız inceleme bulunmalıdır; bu iki kural hem ekranda hem oluşturma sırasında ayrıca doğrulanır. İnceleme ve doğrulama rolleri yalnızca gerçek bir salt-okunur moda sahip bağlantılara verilebilir ve o oturumlar salt okunur açılır. Üretim rolleri aynı izole checkout'ta çalışır; her adım kendi hesabında kendi oturumunu açar, çünkü bir oturum onu başlatan hesaba aittir. **Plan** adımı yalnızca plan modu entegrasyona gömülü olan bağlantılarda (Codex, Claude, OpenCode) o modu ister; bir ACP ya da terminal bağlantısı modlarını ancak çalışırken bildirdiği için orada adım bağlantının kendi varsayılan modunda koşar ve planlama talimatını prompt taşır — bildirilmemiş bir mod istemek turu doğrudan başarısız eder. Sağlayıcının kendi subagent'ları, onları çalıştıran adımın oturumunda görünür.

Seçili iş akışının başlığının altındaki şerit adım sırasını, her adımın hesabını ve başladığı revizyonu salt okunur gösterir; **Run** düğmesi sıradaki adımı adıyla çalıştırır, başarısız bir adım **Retry** olur. Çalışma alanı inceleme sonrası değişirse o inceleme adımı yeniden sıraya döner: doğrulama ve entegrasyon yalnızca incelenen ağaç üzerinde yapılabilir. **Stop** yalnızca iki değil, adım açmış bütün oturumları durdurur.
...
Aynı gerçek checkout kökü için tek aktif agent turuna izin verilir. Monorepo alt dizinleri aynı köke bağlanır. Ayrı checkout/worktree'lerde toplam sekiz aktif tur çalışabilir. Bu kilit RunHQ agent oturumları arasındadır; dış editörler, kullanıcı terminali veya başka uygulamalardaki agent'lar kilide dahil değildir.
