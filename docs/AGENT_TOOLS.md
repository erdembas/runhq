...
## Account pools and routing

**Account pools** on the tools screen group connections you consider interchangeable. RunHQ never infers a pool from a shared adapter, because two accounts for the same product may well belong to different people. A recipe can then target the pool instead of one connection; the task is still created against a single account and keeps it for the whole session, since provider-native resume belongs to the account that opened the conversation.

Three signals choose that account and are kept apart:

- **Capability fit** is declared by the adapter and deterministic. Image attachments go only to Codex and Claude connections, steering only to Codex, and a plan step only to an integration whose plan mode is built in — an ACP or terminal connection advertises its modes once it is running, so plan work is never routed to it on a guess.
- **Quota** is an event, never a forecast. A rate or usage failure the provider actually returned puts that account on a cool-down and routing prefers another account in the pool until it ends. Nothing else starts a cool-down — not an authentication failure, not token counts, not elapsed time, not RunHQ's own slot and queue messages. These CLIs do not publish a remaining allowance, so none is shown, estimated, or implied, and the cool-down is RunHQ's own backoff rather than a reset time claimed on the provider's behalf. **Usage** lists every cool-down with the provider's own words and a **Resume now** control.
- **Load** is the execution slots already counted per connection. Among the accounts that remain, the one with the most free slots runs the task, and the pool's own order breaks a tie so the choice is reproducible. A workspace-wide limit is reported as a global wait rather than blamed on an account.

The task composer's agent menu and a workflow's implementation and review roles list pools under **Account pools**. Choosing one resolves it to an account immediately, because those screens work in connections and discover one account's models and modes; the resolved identity is shown before anything starts and you can still override it. A workflow's review role resolves against review-capable connections only, so a mixed pool cannot put a reviewer where review is not supported. When no account in the pool can take the work, the agent is left unset for you to choose — you are there, unlike a scheduled run.

Whenever RunHQ rather than you picked the account, the task records which account and on what grounds, and the conversation says so above its tabs. Choosing a connection by hand records nothing: that decision is yours and needs no explanation. A connection that is not in a pool is routed the same way as a pool of one, so a single account reports the same reasons without inventing a group. A task that is already running is never moved: taking over after a limit means starting a new session through **Hand off**. When the source account is on cool-down and belongs to a pool, that handoff opens on the account routing would pick; in every other case the agent is left for you to choose.
...
Cursor todo notifications are merged or replaced according to their protocol flag, persisted for later turns, and displayed as plan progress. Subagent notifications become their own transcript items showing the reported description, subagent type, model and duration under the task that owns them, and state that the subagent ran inside Cursor; image notifications display the reported file path. These notifications do not launch additional RunHQ sessions or render local image files automatically. Session-load replay does not duplicate questions, plans or progress.
