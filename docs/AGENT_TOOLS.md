...
**Add tool** accepts a name, executable and connection type. For ACP or terminal connections, each arguments-field line is passed as one argument, including embedded spaces. No shell parsing, command substitution, package installation or credential copying occurs. Configure environment/authentication through the tool’s existing supported setup. There are no hardcoded claims about third-party model availability.
