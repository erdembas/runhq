...
Each start records which account ran it and why. A connection that is not in a pool is routed the same way as a pool of one, so a single account reports the same reasons without inventing a group. A task that is already running is never moved: taking over after a limit means starting a new session through **Hand off**. When the source account is on cool-down and belongs to a pool, that handoff opens on the account routing would pick; in every other case the agent is left for you to choose.
