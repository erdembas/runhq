...
Drafting a pooled recipe into the task composer, or into a workflow, resolves it to that account first, because those screens work in connections and discover one account's models and modes. When no account in the pool can take the work, the agent is left unset for you to choose — you are there, unlike a scheduled run. Each start records which account ran it and why. A connection that is not in a pool is routed the same way as a pool of one, so a single account reports the same reasons without inventing a group. A task that is already running is never moved: taking over after a limit means starting a new session through **Hand off**. When the source account is on cool-down and belongs to a pool, that handoff opens on the account routing would pick; in every other case the agent is left for you to choose.
