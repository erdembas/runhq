...
Each start records which account ran it and why. A connection that is not in a pool is routed the same way as a pool of one, so a single account reports the same reasons without inventing a group. A task that is already running is never moved: taking over after a limit means starting a new session through **Hand off**.
