# Historical Product Backlog

Archived on 2026-09-17 when RunHQ made centralized agent management its primary product focus.
The [active roadmap](../ROADMAP.md) defines current priorities and implementation status.

The original 40-item roadmap follows to preserve feature numbers, scope and rationale.
Its status labels, technical assumptions and implementation order are historical: several items
marked Planned or Proposed are already partly or fully implemented. They are not current commitments.
For current agent behavior and provider boundaries, see [Agent workspace](AGENT_WORKSPACE.md)
and [Agent tools](AGENT_TOOLS.md).
