...
import { buildAppActions } from './items/appActions';
import { buildExpandedStackItems, buildRootStackItems } from './items/stackItems';
...
export function buildItems(deps: BuildItemsDeps): ListItem[] {
  ...
  if (expandedStack) {
    return buildExpandedStackItems(expandedStack, { query, services, getCmds });
  }

  result.push(...buildAppActions({ query, filter, hide, focusMainWindow }));
  result.push(...buildRootStackItems({ query, filter, stacks, services, getCmds }));
