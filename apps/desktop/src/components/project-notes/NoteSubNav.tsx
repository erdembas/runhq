    ...
    <aside className="border-border/60 bg-surface-raised/20 flex w-[200px] shrink-0 flex-col border-r">
      <div className="border-border/60 flex shrink-0 items-center justify-between gap-2 border-b px-2 py-1.5">
        ...
        <button
          type="button"
          onClick={onCreate}
          title={i18n.t('New note (creates an `untitled` markdown file)')}
          aria-label={i18n.t('Create new note')}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto py-1">
        {loading ? (
          <div className="text-fg-dim px-2 py-2 text-[11px]">{i18n.t('Loading…')}</div>
        ) : notes.length === 0 ? (
          <div className="text-fg-muted px-2 py-3 text-[11px] leading-relaxed">
            {i18n.rich('No notes yet.{value1}Press {value2} to create one.', {
              value1: <br />,
              value2: (
