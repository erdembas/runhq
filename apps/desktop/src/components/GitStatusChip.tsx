...
export function GitStatusChip({ serviceId, compact }: { serviceId: ServiceId; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (wrapRef.current) {
      ...
      const above = rect.bottom + 380 > window.innerHeight;
      setPos(
        above
          ? { bottom: window.innerHeight - rect.top + 6, left: rect.right - 360 }
          : { top: rect.bottom + 6, left: rect.right - 360 },
      );
    }
      ...
      {open &&
        createPortal(
          <>
            ...
            />
            <div
              className="border-border bg-surface-raised rounded-app-lg animate-fade-in fixed z-[9999] flex max-h-[calc(100vh-96px)] w-[360px] flex-col overflow-y-auto border shadow-[0_12px_40px_rgba(0,0,0,0.45)]"
