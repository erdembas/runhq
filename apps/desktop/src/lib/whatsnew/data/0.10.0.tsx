/**
 * RunHQ 0.10.0 release highlights — document-style.
 *
 * Two threads weave through this chapter:
 *
 *   • **Per-project context** — every service grows two new sibling
 *     tabs (a private Monaco notebook + a read-only repo twin) and
 *     gets a license scanner with dashboard chips + AI contamination
 *     triage. Together they answer "what do I know about this
 *     project?" without ever touching the repo.
 *
 *   • **Terminal stack hardening** — logs and the integrated terminal
 *     converge on a single xterm.js engine, get a browser-grade
 *     `Cmd/Ctrl+F` shared between them, kill entire descendant trees
 *     on Windows via Job Objects, and stop being silently
 *     bash-hostile when launching shell panes.
 *
 * This is the first release authored under the `DocumentRelease`
 * schema (introduced alongside the page redesign): the modal becomes
 * a slim "tease + jump to full notes" surface, and the content lives
 * here as scrollable long-form prose with an in-page TOC, à la VS
 * Code release notes / Linear changelog.
 *
 * Asset hosting:
 *
 *   • Only `project-notes` carries an inline `<InlineMedia>` clip,
 *     hosted on Cloudflare R2 (`cdn.runhq.dev`). It's the headliner
 *     — every other subsection lands as prose + chips + callouts.
 *     The previous plan (also media for `project-docs-tab` and
 *     `license-scanner`) was trimmed because the document layout
 *     already gives those subsections enough room to tell their
 *     story in copy; an extra video would push prose under the
 *     fold without adding clarity.
 *
 *   • Capture brief for the clip: a service NOTES tab pre-staged
 *     with three notes (Deployment / On-call / Index), Beat 1 types
 *     into Monaco + ⌘B bolds, Beat 2 toggles Preview wide-layout,
 *     Beat 3 opens AI Chat (⌘L) and asks "what runs after seed?"
 *     so the model quotes back the freshly-typed `prisma migrate
 *     deploy` line. ~6-8 s, dark theme, native 1080p.
 *
 *   • Upload (one-shot, after capturing + encoding to `webm` via
 *     `ffmpeg -i in.mov -an -c:v libvpx-vp9 -crf 31 -b:v 0 …`):
 *
 *       pnpm dlx wrangler r2 object put \
 *         runhq-cdn/whatsnew/0.10.0/project-notes.webm \
 *         --file ./project-notes.webm \
